...
y_position = 176; % y position of _BONE.GOBJ file
firstslice = 32; % z position + 1 of _BONE.GOBJ file


%files = dir('*AIM;1');
dirName = 'DK0:[MICROCT.DATA.00002000.00026719]';
...
for i = 1:1% length(files)
    ...
    fprintf(fid,'\n%s',' ');
    fprintf(fid,'\n%s','sub_get','trabseg','trab_sub',['5 0 ' num2str(firstslice)],'400 400 100','false'); % Account for +1 extra position in z direction
    fprintf(fid,'\n%s',' ');
...
%     fprintf(fid,'\n%s',' ');
    ...
    fprintf(fid,'\n%s',' ');
    fprintf(fid,'\n%s','sub_get','trabout','trabout2','5 0 1','400 400 100','false');
    fprintf(fid,'\n%s',' ');
...
%     fprintf(fid,'\n%s',' ');
   ...
   % fprintf(fid,'\n%s','bound','cartseg','out','true','0 0 0');
    ...
    fprintf(fid,'\n%s',' ');
    fprintf(fid,'\n%s','sub_get','cartseg','cart_out',['5 0 ' num2str(firstslice)],'400 400 100','false');
    fprintf(fid,'\n%s',' ');
